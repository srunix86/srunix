...
void execute_fetch() {
    ...
    uint8_t colors[] = {
        COLOR_RED, COLOR_GREEN, COLOR_YELLOW, COLOR_BLACK, 
    ...
    terminal_setcolor(COLOR_WHITE, COLOR_BLACK);
    terminal_writestring(" Srunix86 v0.0.5-1 amd64\n");
    terminal_setcolor(COLOR_YELLOW, COLOR_BLACK);
    ...
    terminal_setcolor(COLOR_WHITE, COLOR_BLACK);
    terminal_writestring(" Srunix Kernel 0.0.5-1\n");
    terminal_setcolor(COLOR_YELLOW, COLOR_BLACK);
    terminal_writestring(" |___/_|_| _  License:");
    terminal_setcolor(COLOR_WHITE, COLOR_BLACK);
    terminal_writestring(" BSD license\n");
    terminal_setcolor(COLOR_YELLOW, COLOR_BLACK);
    ...
    terminal_setcolor(COLOR_WHITE, COLOR_BLACK);
    terminal_writestring(" ush 6.0-02\n");
    terminal_setcolor(COLOR_YELLOW, COLOR_BLACK);
...
void execute_command(char* cmd) {
    ...
    if (strcmp_case_insensitive(args[0], "help") == 0) {
        ...
        execute_uptime();
    } else if (strcmp_case_insensitive(args[0], "sh") == 0 ||
	       strcmp_case_insensitive(args[0], "srunix64") == 0 ) {
	    shell();
        ...
        terminal_setcolor(COLOR_WHITE, COLOR_BLACK);
        info_program_run();
    } else if (strcmp_case_insensitive(args[0], "dhcpcd") == 0) {
        if (arg_count > 1) execute_dhcpcd(args[1]);
        else terminal_writestring("Usage: dhcpcd <interface>\n");
        ...
        else terminal_writestring("Usage: ping <host>\n");
        ...
        execute_beep();
    } else if (strcmp_case_insensitive(args[0], "Z") == 0) {
	    zv();
...
void print_prompt() {
    terminal_setcolor(COLOR_BRIGHT_RED, terminal_color >> 4);
    terminal_writestring("root");
    terminal_setcolor(COLOR_WHITE, terminal_color >> 4);
    terminal_writestring(":");
    if (current_inode != 1) {
	terminal_setcolor(COLOR_BRIGHT_BLUE, terminal_color >> 4);
        terminal_writestring("~");
...
void login_screen() {
    ...
    terminal_clear();
    terminal_writestring("\n\n");
    terminal_writestring("Srunix86 tty");
    char tty_num[2];
    int_to_str(current_tty + 1, tty_num);
    terminal_writestring(tty_num);
    terminal_writestring("\n");
